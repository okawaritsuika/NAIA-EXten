...
## v0.4.1 - Comic Maker 작업 분리와 Parquet 동기화 개선

NAIA 본체 소스를 교체하지 않고 EXten에서 적용하는 업데이트입니다. 확장을 업데이트한 뒤 다시 불러오고, **Ctrl+R**로 화면을 새로고침합니다.

### Comic Maker · NAI 생성 / 저장 계획

- **만화 만들기 (NAI)**: 모달에서 일반 만화 또는 큰 화면을 선택합니다. 일반 만화는 페이지당 한 장, 큰 화면은 패널당 한 장을 현재 NAIA 해상도로 생성합니다.
- **만화 불러오기 (저장 계획)**: 현재 캐릭터 인원수에 맞는 저장된 ComicPlan을 서버에서 랜덤으로 불러옵니다. 계획 정보를 확인한 뒤 **저장 계획으로 만들기**를 눌러 이미지 생성을 시작합니다.

NAI 생성과 저장 계획은 캐릭터·해상도 설정과 진행 상태를 각각 관리합니다. **NAI 작업 중지**와 **저장 계획 작업 중지**는 해당 작업만 중지합니다. 이미지 요청은 기존 NAIA 큐가 순서대로 처리합니다.

### Parquet 실시간 동기화

- 사용한 행을 작은 SQLite 기록에 즉시 저장하고, 원본 Parquet의 삭제는 15초 단위로 모아서 반영합니다.
- 종료·재시작이나 파일 전환 후에도 미반영 기록을 유지합니다. 이전 검색 결과를 복원해도 이미 사용한 행이 다시 선택되지 않도록 처리합니다.
- 대용량 Parquet은 나눠 읽어 처리하며, 원래 열 형식과 메타데이터를 유지합니다. 파일 반영에 실패하면 기록을 보관해 다음 주기에 다시 처리합니다.
- 다중 Parquet 균등 선택도 같은 사용 기록에 연결하고, 파일을 불러올 때 불필요한 전체 복사를 줄였습니다.

`parquet_consumption.sqlite3`와 동반 파일은 개인 사용 기록이므로 Git에 포함하지 않습니다. 로컬 백업 폴더도 배포 대상에서 제외합니다.

## 생성 진행 막대 표시

EXten 퀵 팝업의 **화면 표시 → 생성 진행 막대**에서 선택합니다.

- **부드럽게** (기본): NAIA의 예상 생성 시간을 유지하면서 막대가 연속적으로 움직입니다.
- **숨기기**: 위쪽 진행 막대를 숨기고 막대 갱신도 멈춥니다.
- **NAIA 기본**: NAIA 원래 표시 방식으로 돌아갑니다.

선택은 저장되며 생성 중에도 변경할 수 있습니다. 이미지 생성 요청이나 결과에는 영향을 주지 않습니다.
처음 코드를 반영한 뒤에는 **Ctrl+R 한 번**으로 화면을 새로고침해야 합니다.
